...
for rel in ['vendor/zip-reader.js','resource-db.js','rose-cookies-manager.js','data/rose-cookies-schema.js']:
 r=subprocess.run(['node','--check',str(ROOT/rel)],capture_output=True,text=True);ck(r.returncode==0,f'{rel} JavaScript syntax')
...
version=json.loads((ROOT/'VERSION.json').read_text(encoding='utf8'));ck(version['version']=='0.5.0' and version['annotationSchemaVersion']=='0.4.0','v0.5 app retains annotation schema v0.4')
