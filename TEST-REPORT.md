...
### Static / integrity suite — 29/29 passed
...
- v0.2 annotation schema parses and reports the correct schema version.
- The exact uploaded v0.1.1 migration sample contains the expected 5 groups, 22 components and 6 structures.
- Required v0.2 implementation hooks exist for unique IDs, migration, duplicate/conflict/unattached validation, Hebrew coverage validation, revision restore and segment assignment.
...
### Runtime startup + migration/workflow smoke suite — 24/24 passed
...
- editing an approved component resets the edited revision to **Needs review**;
...
`sample/PSA001.annotations.v0.2.migrated-preview.json` is the v0.2 migration output produced from the uploaded v0.1.1 Psalm 1 annotations before the workflow mutation tests begin.
